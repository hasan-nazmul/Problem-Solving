...
void solve()    {
    ...
    ll l, l1, l2, r1, r2, M, N, O, P, m = 0, val = 0, num = 0, n = 0, o = 0, p = 0, q, r, i, j, k = 1, count = 0, sum = 0, min = LONG_MAX, max = LONG_MIN, index = 0, diff;
    ...
    deque <ll> dq, iq, eq, odd, even, cnt(0, 0), chk;

    deque <pair <ll, ll>> pdq, qdq, ans;
    ...
    map <ll, pair <ll, ll>> mp, mpX, mpY, emp;
    ...
    while(1)
    {
        ...
        for(i = 0; i < sz(divisors); i++)
        {
            for(j = i; j < sz(divisors); j++)
            {
                if(__lcm(divisors[i], divisors[j]) == N)    count++;
            }
        }
