...
const int mx_len = 110;
 
ll n, x;
 
ll coins[mx_len], dp[1000010];
...
ll calc(ll amount)
{
    ...
    for(int i = 0; i < n; i++)
        ...
        {
            if(dp[amount] != -1)
                dp[amount] = min(dp[amount], 1+calc(amount-coins[i]));
 
            else    dp[amount] = 1+calc(amount-coins[i]);
        }
 
    return dp[amount]==-1 ? INT_MAX:dp[amount];
}
