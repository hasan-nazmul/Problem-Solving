...
void solve()
{
    ...
    while(q--)
    {
        ...
        set <vector <ll>> updated_pos;

        if(v[l]+1 <= n)
            updated_pos.insert({v[l], v[l]+1});
        if(v[l]-1 > 0)
            updated_pos.insert({v[l]-1, v[l]});
        if(v[r]+1 <= n)
            updated_pos.insert({v[r], v[r]+1});
        if(v[r]-1 > 0)
            updated_pos.insert({v[r]-1, v[r]});

        for(auto i : updated_pos)
            cnt -= (int) (pos[i[0]] > pos[i[1]]);
